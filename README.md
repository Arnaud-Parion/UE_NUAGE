...
- main qui comporte la partie kubernetes seul, et terraform avec kubernetes 
- dockercompose-terraform_docker qui comporte la partie terraform docker

Pour la suite du projet on suppose que docker fonctionne (lancer docker desktop si sur Windows)
...
Pour cette partie, allez dans la branche dockercompose-terraform_docker et lancez les commandes suivantes :
  - ```terraform init```
  - ```terraform plan```
  - ```terraform apply```
  - "yes"
...
Il faut installer Google Cloud CLI et terraform.
...
Ensuite modifiez les configurations gcp avec les informations suivantes : 
```provider "google" {```
```  project = "project-id"```
```  region  = "europe-west9"```
```  zone    = "europe-west9-a"```
